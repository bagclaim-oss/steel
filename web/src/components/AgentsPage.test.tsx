...
describe("AgentsPage", () => {
  ...
  it("clicking Copy URL writes the webhook URL to the clipboard", async () => {
    // Validates the happy path: the visible Copy URL action writes the computed
    // webhook URL to navigator.clipboard and reflects success in the UI.
    ...
    await waitFor(() => {
      expect(writeText).toHaveBeenCalledWith(
        `${window.location.origin}/api/agents/a1/webhook/secret123`,
      );
    });
    expect(screen.getByText("Copied!")).toBeInTheDocument();
  });

  it("clicking Copy URL falls back to execCommand when clipboard API fails", async () => {
    // Edge case: browser denies clipboard access (e.g. insecure context or blocked permission).
    // The component should still copy via the legacy document.execCommand fallback.
