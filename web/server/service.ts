...
const STDOUT_LOG = join(LOG_DIR, "steel.log");
const STDERR_LOG = join(LOG_DIR, "steel.error.log");

// ─── macOS (launchd) Constants ──────────────────────────────────────────────────

const LABEL = "ai.steel.app";
const OLD_LABEL = "sh.thecompanion.app";
...
export function generateSystemdUnit(opts: UnitOptions): string {
  ...
  return `[Unit]
Description=Steel - Web UI for Claude Code
...
function resolveBinPath(): string {
  try {
    const binPath = execSync("which steel", { encoding: "utf-8" }).trim();
    if (binPath) return binPath;
  } catch {
    // not found globally
  }

  console.error("steel must be installed globally for service mode.");
  console.error("");
  console.error("  bun install -g steel");
  console.error("");
  console.error("Then retry:");
  console.error("");
  console.error("  steel install");
  process.exit(1);
}
...
async function installDarwin(opts?: { port?: number }): Promise<void> {
  ...
  if (existsSync(PLIST_PATH)) {
    console.error("Steel is already installed as a service.");
    console.error("Run 'steel uninstall' first to reinstall.");
    process.exit(1);
  }
  ...
  console.log("Steel has been installed as a background service.");
  console.log("");
  ...
  console.log("Use 'steel status' to check if it's running.");
}
...
async function uninstallDarwin(): Promise<void> {
  ...
  if (!installedService) {
    console.log("Steel is not installed as a service.");
    return;
  }
  ...
  console.log("Steel service has been removed.");
...
async function startDarwin(): Promise<void> {
  ...
  if (!installedService) {
    console.log("Steel is not installed as a service.");
    console.log("Run 'steel install' first.");
    return;
  }
  ...
  console.log("Steel service has been started.");
}
...
async function stopDarwin(): Promise<void> {
  ...
  console.log("Steel service has been stopped.");
  console.log("Run 'steel restart' to start it again.");
}
...
async function restartDarwin(): Promise<void> {
  ...
  console.log("Steel service has been restarted.");
}
