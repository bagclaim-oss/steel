...
describe("getRepoInfo", () => {
  ...
  it("detects worktree when git-dir contains /worktrees/", () => {
    mockGitCommands({
      "rev-parse --show-toplevel": "/fake/home/.steel/worktrees/proj/feat--x",
      "rev-parse --abbrev-ref HEAD": "feat/x",
      "rev-parse --git-dir": "/home/user/proj/.git/worktrees/feat--x",
      "symbolic-ref refs/remotes/origin/HEAD": "refs/remotes/origin/main",
    });

    const result = gitUtils.getRepoInfo("/fake/home/.steel/worktrees/proj/feat--x");
...
describe("listWorktrees", () => {
  it("parses porcelain output correctly", () => {
    const porcelain = [
      ...
      "",
      "worktree /fake/home/.steel/worktrees/project/feat--x",
    ...
    expect(worktrees[1].path).toBe("/fake/home/.steel/worktrees/project/feat--x");
  });
...
describe("ensureWorktree", () => {
  ...
  it("creates worktree for an existing local branch", () => {
    ...
    expect(result.worktreePath).toBe("/fake/home/.steel/worktrees/repo/feat--local");
  ...
  it("calls mkdirSync with recursive option when creating worktree", () => {
    ...
    expect(mockMkdirSync).toHaveBeenCalledWith(
      "/fake/home/.steel/worktrees/repo",
      { recursive: true },
    );
  });
  ...
  it("does not reuse the main worktree even when branch matches", () => {
    ...
    expect(result.worktreePath).toBe("/fake/home/.steel/worktrees/repo/main");
  ...
  it("creates unique paths with random suffix when base path exists", () => {
    ...
    const basePath = "/fake/home/.steel/worktrees/repo/feat--x";
  ...
  it("creates branch-tracking worktree when forceNew=true and worktree already exists", () => {
    ...
    expect(result.worktreePath).toBe("/fake/home/.steel/worktrees/repo/feat--existing");
    expect(result.branch).toBe("feat/existing");
