...
function plistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "ai.steel.app.plist");
}
...
function unitPath(): string {
  return join(tempDir, ".config", "systemd", "user", "steel.service");
}
...
describe("generatePlist", () => {
  it("generates valid XML with the correct label", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/steel" });
    expect(plist).toContain('<?xml version="1.0"');
    expect(plist).toContain("<string>ai.steel.app</string>");
  });
  ...
  it("uses the provided binary path in ProgramArguments", () => {
    const plist = service.generatePlist({ binPath: "/opt/homebrew/bin/steel" });
    expect(plist).toContain("<string>/opt/homebrew/bin/steel</string>");
  ...
  it("uses a custom port when specified", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/steel", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/steel", path: customPath });
...
describe("generateSystemdUnit", () => {
  it("generates a valid systemd unit with correct sections", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/steel" });
  ...
  it("includes the description", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/steel" });
    expect(unit).toContain("Description=Steel");
  });

  it("uses the provided binary path in ExecStart", () => {
    const unit = service.generateSystemdUnit({ binPath: "/home/user/.bun/bin/steel" });
    expect(unit).toContain("ExecStart=/home/user/.bun/bin/steel start --foreground");
  });
  ...
  it("uses a custom port when specified", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/steel", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    const customPath = "/custom/bin:/other/bin";
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/steel", path: customPath });
...
describe("install", () => {
  it("creates log directory and writes plist file", async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/usr/local/bin/steel\n";
    ...
    expect(content).toContain("ai.steel.app");
  });
  ...
  it("migrates old launchd label before installing", async () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/steel" })
      .replaceAll("ai.steel.app", "sh.thecompanion.app");
...
describe("install (linux)", () => {
  ...
  it("creates log directory and writes systemd unit file", async () => {
    ...
    expect(content).toContain("ExecStart=/usr/local/bin/steel start --foreground");
  });
...
describe("start (linux)", () => {
  ...
  it("calls systemctl start when installed", async () => {
    ...
    const startCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("start steel.service"),
    );
    expect(startCall).toBeDefined();
  });
...
describe("stop (linux)", () => {
  ...
  it("calls systemctl stop when installed", async () => {
    ...
    const stopCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("stop steel.service"),
    );
    expect(stopCall).toBeDefined();
  });
...
describe("restart (linux)", () => {
  ...
  it("calls systemctl restart when installed", async () => {
    ...
    const restartCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("restart steel.service"),
    );
    expect(restartCall).toBeDefined();
  });
...
describe("status (linux)", () => {
  ...
  it("returns installed: true, running: true with PID", async () => {
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (typeof cmd === "string" && cmd.includes("show steel.service")) {
...
describe("refreshServiceDefinition (macOS)", () => {
  it("rewrites plist with current binary path", async () => {
    ...
    expect(originalContent).toContain("/usr/local/bin/steel");
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/new/path/steel\n";
      return "";
    });
    ...
    expect(updatedContent).toContain("/new/path/steel");
  });
